...
2. **Every KEEP clears a Bonferroni-corrected bar, not a flat 0.05.** `results/feature_hypothesis_log.csv` counts every feature hypothesis ever spent; the bar is `alpha = 0.05 / family_size` (now `0.05/5 = 0.01` — hypothesis 5 was the rejected 2026-07-17 `fomc_calendar_block` ADD-test), printed in every `src/ablation.py` report. Register a genuinely new feature (it tightens the bar for all). All 4 current features are **KEEP-provisional** — no proven edge.
